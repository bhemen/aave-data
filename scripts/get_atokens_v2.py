...
api_url = 'http://localhost:8545' #Put in your local node
provider = HTTPProvider(api_url)
web3 = Web3(provider)

#Addresses
v2_data_provider_address = "0x057835Ad21a177dbdd3090bB1CAE03EaCF78Fc6d"
v3_data_provider_address = "0x7B4EB56E7CD4b454BA8ff71E4518426369a138a3"

protocols = { 'v2': v2_data_provider_address, 'v3': v3_data_provider_address }
DECIMAL_ABI = json.loads('[{"inputs":[],"name":"decimals","outputs":[{"internalType":"uint8","name":"","type":"uint8"}],"stateMutability":"view","type":"function"}]')

for v, a in protocols.items():
	aave_protocol_data_provider_address = Web3.to_checksum_address(a)
	...
	#Adding decimals for tokens
	decimals = {}
	#Unable to get decimals from UST contract, so adding it manually
	decimals['UST'] = 6
	decimals['cbETH'] = 18
	for symbol, address in reserve_tokens:
		#token_abi = get_cached_abi(address)
		token_contract = web3.eth.contract( address=address, abi=DECIMAL_ABI )
		try:
			decimals[symbol] = token_contract.functions.decimals().call()
		except Exception as e:
			print( "Error" )
			print( f"Contract = { address } ({symbol})" )
			print( f"ABI = {token_contract.abi}" )
			print( e )
	...
	for symbol,address in reserve_tokens:
		...
		# Calling getReserveTokensAddresses function from Protocol Data Provider Contract => https://docs.aave.com/developers/v/2.0/the-core-protocol/protocol-data-provider#getreservetokensaddresses
	...
	df.to_csv( f"../data/aave_atokens_{v}.csv",index=False)
