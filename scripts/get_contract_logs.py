...
def getContractEvents( contract_address, target_events, outfile, start_block=1,end_block=None ):
	...
	with progressbar.ProgressBar(max_value=end_block-start_block,widgets=[progressbar.Variable('BatchSize')]) as bar:
		while True:
			...
			if len(lgs) > 0:
				new_rows = processLogs(contract,event_signatures,lgs)	
				df = pd.concat( [df, new_rows] )
				df.to_csv(outfile,index=False)
			
			bar.update(batch_end_block-start_block,BatchSize=batch_size)
...
def processLogs(contract,event_signatures,lgs):
	rows = []
	for lg in lgs:
