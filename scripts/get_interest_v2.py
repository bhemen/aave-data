...
#Get a list of tokens (don't have token transfers for 1Inch and LUSD)
...
#Removed tokens for which we dont have the transfer records
# reserve_tokens_transfer = reserve_tokens[0:-2]
...
for ctr,symbol,address in enumerate(reserve_tokens):
    if ctr == 0:
...
#Merging and saving
new_df = pd.merge_asof(new_df, usd_df, on = 'timestamp', by = 'symbol')
new_df.to_csv("../data/temp_records.csv")
...
for user in tqdm(aave_user_addresses):
    ...
    for symbol in interest_gained.keys():
        if user in interest_gained[symbol].keys():
            if interest_gained[symbol][user]['interestAccrualPeriod'] != 0:
...
for symbol, address in reserve_tokens:
    if len(interest_gained[symbol].keys())>0:
        ...
        final_df.to_csv(f"../data/aave_v2_user_interests/{symbol}_user_interests.csv")