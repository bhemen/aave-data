"""
	...
	This script generates the file data/aave_collateralization_v2.csv
"""
...
outfile = "../data/aave_collateralization_v2.csv"
errorfile = "../data/aave_collateralization_errors_v2.csv"
...
# Iterate over the desired blocks and retrieve collateralization data
with progressbar.ProgressBar(max_value=len(blocks)) as bar:
	...
	for i,block_num in enumerate(blocks):
		...
		for symbol,address in reserve_tokens:
			...
			#Calling getReserveData function from Protocol Data Provider Contract => https://docs.aave.com/developers/v/2.0/the-core-protocol/protocol-data-provider#getreservedata
			try:
			...
			#Calling getReserveNormalizedIncome function from LendingPool Contract => https://docs.aave.com/developers/v/2.0/the-core-protocol/lendingpool#getreservenormalizedincome
			try:
				reserveNormalizedIncome = aave_lending_pool_contract.functions.getReserveNormalizedIncome(address).call(block_identifier=block_num)
			except Exception as e:
				with open( errorfile, "a" ) as f:
					f.write( f"Error calling getReserveNormalizedIncome at {block_num} for asset {symbol}\n" )
				continue

			#Calling getReserveNormalizedVariableDebt function from LendingPool Contract => https://docs.aave.com/developers/v/2.0/the-core-protocol/lendingpool#getreservenormalizedvariabledebt
			try:
				reserveNormalizedVariableDebt = aave_lending_pool_contract.functions.getReserveNormalizedVariableDebt(address).call(block_identifier=block_num)
			except Exception as e:
				with open( errorfile, "a" ) as f:
			...
			row = { 'block': block_num,
					...
					'symbol': symbol,
					'address': address,
					'availableLiquidity': token_info[0],
					'totalStableDebt': token_info[1], 
					'totalVariableDebt': token_info[2],
					'liquidityRate': token_info[3], 
					'variableBorrowRate': token_info[4],
					'stableBorrowRate': token_info[5], 
					'averageStableBorrowRate': token_info[6],
					'liquidityIndex': token_info[7], 
					'variableBorrowIndex': token_info[8],
					'lastUpdateTimestamp': token_info[9],
